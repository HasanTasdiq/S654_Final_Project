...
def init( master_key):
    assert len(master_key) in  rounds_by_key_size
    global n_rounds 
    n_rounds =  rounds_by_key_size[len(master_key)]
    global _key_matrices
    _key_matrices =  _expand_key(master_key)
...
def decrypt_block(  ciphertext):
    ...
    add_round_key(cipher_state,  _key_matrices[-1])
    inv_shift_rows(cipher_state)
    inv_sub_bytes(cipher_state)

    for i in range( n_rounds - 1, 0, -1):
        add_round_key(cipher_state,  _key_matrices[i])
        inv_mix_columns(cipher_state)
        inv_shift_rows(cipher_state)
        inv_sub_bytes(cipher_state)

    add_round_key(cipher_state,  _key_matrices[0])

    return matrix2bytes(cipher_state)

def encrypt_ctr(  plaintext, iv):
    ...
    for plaintext_block in split_blocks(plaintext, require_padding=False):
        ...
        block = xor_bytes(plaintext_block,  encrypt_block(nonce))
        blocks.append(block)
        nonce = inc_bytes(nonce)

    return b''.join(blocks)

def decrypt_ctr(  ciphertext, iv):
    ...
    for ciphertext_block in split_blocks(ciphertext, require_padding=False):
        ...
        block = xor_bytes(ciphertext_block,  encrypt_block(nonce))
        blocks.append(block)
        nonce = inc_bytes(nonce)
...
def encrypt(key, plaintext, workload=100000):
    ...
    print("iv: " , iv)
    ciphertext = encrypt_ctr(plaintext, iv)
...
def decrypt(key, ciphertext, workload=100000):
    ...
    return decrypt_ctr(ciphertext, iv)

def test_AES_ctr():
    data = generate_data()
    key = os.urandom(16)
    init(key)
    print('round key ' , n_rounds )
    ciphertext = encrypt(key , data)
    plaintext = decrypt(key, ciphertext).decode('utf-8')

    print('data: ' , data)
    print('decrypted: ', plaintext)

    print("is equal: " , data == plaintext)
test_AES_ctr()
